...
function syncTouches(touchList){
  ...
  touchHeld = nowHeld;
  for(const b of TOUCH_BTNS){ const el=document.getElementById(b.id);   // 按下高亮反馈
    if(el) el.classList.toggle('held', !!nowHeld[b.code]); }
  return canvasTouch;
}
function onTouch(e){
...
/* ---------------- 背景音乐 (程序化 16 步音序器) ----------------
   每章一段循环旋律 + 贝斯, Boss 战切换更激烈的曲子。
   走独立 music 增益再进 master, 因此受全局音量/静音控制。
   用 Web Audio 提前调度(lookahead)保证节拍稳定, 与渲染帧率无关。*/
const MUSIC = { gain:null, on:false, cur:null, step:0, nextT:0, timer:null, stepDur:0.15 };
const TRACKS = {
  // [melody(16), bass(16)]  0 = 休止
  overworld:[[523,0,659,0,784,0,659,0, 587,0,523,0,440,0,392,0],
             [131,0,0,0,165,0,0,0, 147,0,0,0,131,0,0,0]],
  nether:   [[330,0,0,392,0,0,330,0, 294,0,0,247,0,294,0,0],
             [110,0,110,0,98,0,98,0, 87,0,87,0,98,0,98,0]],
  deepdark: [[0,0,392,0,0,0,330,0, 0,0,440,0,0,0,349,0],
             [65,0,0,0,0,0,0,0, 73,0,0,0,0,0,0,0]],
  end:      [[440,0,587,0,0,523,0,0, 494,0,659,0,0,587,0,0],
             [110,0,0,0,147,0,0,0, 123,0,0,0,98,0,0,0]],
  boss:     [[523,523,0,622,0,523,0,466, 523,0,622,659,0,622,0,587],
             [131,131,131,131,116,116,116,116, 131,131,131,131,156,156,156,156]],
};
function musicSink(){
  if(!actx) return null;
  if(!MUSIC.gain){ MUSIC.gain=actx.createGain(); MUSIC.gain.gain.value=0.55; MUSIC.gain.connect(sink()); }
  return MUSIC.gain;
}
function mtone(freq, at, dur, type, vol){
  try{
    ...
    o.connect(g); g.connect(musicSink()); o.start(at); o.stop(at+dur+0.02);
  }catch(e){}
}
function scheduleStep(step, at){
  const tr=TRACKS[MUSIC.cur]; if(!tr) return;
  const mel=tr[0][step], bas=tr[1][step];
  if(mel) mtone(mel, at, MUSIC.stepDur*0.9, 'square',   0.11);
  if(bas) mtone(bas, at, MUSIC.stepDur*1.4, 'triangle', 0.16);
}
function musicLoop(){
  if(!MUSIC.on || !actx){ return; }
  while(MUSIC.nextT < actx.currentTime + 0.18){
    scheduleStep(MUSIC.step, MUSIC.nextT);
    MUSIC.nextT += MUSIC.stepDur;
    MUSIC.step = (MUSIC.step+1)&15;
  }
  MUSIC.timer = setTimeout(musicLoop, 50);
}
function playMusic(name){
  if(!audio() || actx.state!=='running') return;   // 未解锁前不放, 解锁后由 syncMusic 续上
  if(MUSIC.cur===name && MUSIC.on) return;
  MUSIC.cur=name; MUSIC.stepDur = name==='boss'?0.115:0.15;
  if(!MUSIC.on){ MUSIC.on=true; MUSIC.step=0; MUSIC.nextT=actx.currentTime+0.08; musicLoop(); }
}
function stopMusic(){ MUSIC.on=false; MUSIC.cur=null; if(MUSIC.timer){ clearTimeout(MUSIC.timer); MUSIC.timer=null; } }
// 依据当前游戏状态选择应播放的曲目
function syncMusic(){
  if(!actx || actx.state!=='running'){ return; }
  if(G.state==='play' && !G.paused && player && !player.dead){
    let want;
    if(G.mode==='bossrush' || G.mode==='endless') want='boss';
    else { const wkey=['overworld','nether','deepdark','end'][G.levelIndex]||'overworld';
      want=(boss && !boss.defeated && boss.state==='fight')?'boss':wkey; }
    playMusic(want);
  } else if(G.state==='play' && G.paused){
    // 暂停: 维持当前调度但其实节拍继续 — 简单起见停掉, resume 时续上
...
/* ---------------- 关卡数据 ----------------
   ...
   '=' 木板平台  'n' 下界岩  'e' 末地石
   'J' 黏液块(踩上弹高)  'G' 灵魂沙(踩上减速)  'I' 蓝冰(打滑)
   'L' 岩浆(致命)  '^' 尖刺/仙人掌(致命)  'T' TNT机关  'P' 压力板机关
   'o' 绿宝石(加分)  'A' 金苹果(回血)
   'V' 速度药水  'W' 力量药水  'Y' 抗火药水  'U' 不死图腾
   'z' 僵尸  'c' 苦力怕  'k' 骷髅
   'm' 史莱姆  'x' 末影人  'f' 烈焰人  'r' 蜘蛛
   '@' 出生点  'X' Boss 触发  'F' 终点传送门
*/
const SOLID = new Set(['#','d','s','b','=','n','e','J','G','I']);
const DEADLY = new Set(['L','^']);
const ICE_TILE='I', SOUL_TILE='G', SLIME_TILE='J';   // 特殊地面属性
// 返回实体脚下站立的瓦片字符(用于地面属性: 弹跳/减速/打滑)
function standingTile(e){
  const r=Math.floor((e.y+e.h+1)/TILE);
  if(r<0||r>=G.lvl.H) return ' ';
  const c1=Math.floor((e.x+3)/TILE), c2=Math.floor((e.x+e.w-3)/TILE);
  for(let c=c1;c<=c2;c++){ const ch=G.lvl.map[r][c]; if(ch&&ch!==' ') return ch; }
  return ' ';
}
...
"nnnnnn ### nGGn  TT  nnnnnnnn  nnnn  nnnnnn  TT  nnnnnnnnn  nnnnn  nnnnnnn  TT  nnnnnnnn   nnnnn",
...
"sssss ### sssssssss  TT  ssssssss  ssssss  TT  ssssssss  IIII  sssssss  TT  ssssss   sssssssss ",
...
"                  =====                          =====                        A               ",
"     =====                  ===              ===            =====                              ",
"     x      k    m    o o          k   k    U       o o            k                           ",
"   @     o      ===          ^^^         ===   o         ===            ^^^^        o          ",
"  eeee      eeeeee      eeee      eeeeee     IIII      eeeeee      eeee      eeee      eeeeeeee ",
...
function buildLevel(def){
  ...
  const lvl = { def, W, H, map, pxW:W*TILE, pxH:H*TILE,
                emeralds:[], apples:[], items:[], spawns:[], bossTrigger:null, start:{x:80,y:80} };
  const MOB = { z:'zombie', c:'creeper', k:'skeleton', m:'slime', x:'enderman', f:'blaze', r:'spider' };
  const POT = { V:'speed', W:'strength', Y:'fireres', U:'totem' };
  for(let r=0;r<H;r++) for(let c=0;c<W;c++){
    ...
    else if(POT[ch]){ lvl.items.push({x:px+8,y:py+6,w:24,h:30,t:0,got:false,kind:POT[ch]}); map[r][c]=' '; }
    else if(MOB[ch]){ lvl.spawns.push({type:MOB[ch],x:px,y:py}); map[r][c]=' '; }
...
const G = {
  ...
  menuIndex:0,          // 菜单选中项
  // v1.3 juice
  hitStop:0,            // 顿帧: >0 时冻结逻辑若干"真实帧"以强化打击感
  look:0,               // 摄像机前瞻偏移(朝向缓动)
  camSnap:true,         // 关卡/Boss切换后首帧摄像机直接吸附, 不滑入
  flash:0, flashColor:'255,255,255',  // 全屏闪光(爆炸/击败)
  combo:0, comboT:0,    // 连击数 / 连击计时
  // v1.5 模式
  mode:'story',         // story | bossrush | endless
  titleSel:0,           // 标题模式选择
  brIndex:0, brTime:0,  // Boss Rush: 当前 Boss 序号 / 计时
  wave:0,               // 无尽: 波数
  // v1.6 过场
  fade:0, fadeDir:0, fadeNext:null,   // 关卡转场淡入淡出
};
const BOSSES = ['creeper','wither','warden','dragon'];
const ARENA_THEMES = [
  { name:'竞技场 · 草原', boss:'creeper', sky:['#79c0ff','#bfe6ff'], floor:'#' },
  { name:'竞技场 · 下界', boss:'wither',  sky:['#3a0d0d','#7a1f12'], floor:'n' },
  { name:'竞技场 · 深暗', boss:'warden',  sky:['#05080d','#0d1b2a'], floor:'s' },
  { name:'竞技场 · 末地', boss:'dragon',  sky:['#0a0010','#241038'], floor:'e' },
];
let player, enemies, projectiles, particles, boss, traps, floaters, floatTexts;
...
function makePlayer(x,y){
  return { x,y,w:26,h:36, vx:0,vy:0, dir:1, onGround:false,
           ...
           dblL:0, dblR:0,        // 双击方向→冲刺(移动端无 Shift)的计时窗
           speedT:0, strengthT:0, fireresT:0, totem:false,   // v1.4 药水/图腾
           win:false };
}
...
function touchingSpikes(e){   // 仅尖刺 '^' (抗火免疫岩浆但仍怕尖刺)
  const c1=Math.floor((e.x+4)/TILE), c2=Math.floor((e.x+e.w-4)/TILE);
  const r1=Math.floor((e.y+4)/TILE), r2=Math.floor((e.y+e.h-2)/TILE);
  for(let r=r1;r<=r2;r++) for(let c=c1;c<=c2;c++)
    if(r>=0&&r<G.lvl.H&&c>=0&&c<G.lvl.W && G.lvl.map[r][c]==='^') return true;
  return false;
}
...
function spawnEnemy(s){
  ...
  if(s.type==='slime'){ const sz=s.size||3; base.size=sz; base.w=10+sz*8; base.h=10+sz*8;
    base.hp=sz; base.speed=0.8+sz*0.15; base.hop=rand(20,50); base.y=s.y+(24-base.h); }
  if(s.type==='enderman'){ base.w=24; base.h=46; base.hp=3; base.speed=1.3; base.tele=rand(70,130); }
  if(s.type==='blaze'){ base.w=30; base.h=34; base.hp=3; base.fly=true; base.shoot=rand(50,90);
    base.baseY=s.y; }
  if(s.type==='spider'){ base.w=38; base.h=22; base.hp=2; base.speed=1.8; base.climb=0; }
  enemies.push(base);
}

function updateEnemy(e){
  e.t++; e.anim+=Math.abs(e.vx)*0.2; if(e.flash>0) e.flash--;
  if(e.type!=='blaze'){ e.vy += 0.6; if(e.vy>14) e.vy=14; }   // 重力(烈焰人飞行除外)
  ...
  /* ---- 史莱姆: 朝玩家跳跃, 击杀时分裂 ---- */
    ...
    if(e.onGround){ e.vx*=0.8;
      if(e.hop<=0){ e.hop=rand(34,64); e.dir=sign(dist)||e.dir; e.vx=e.dir*e.speed*2.2; e.vy=-7-e.size*0.8; } }
    e.hitWall=false; moveEntity(e);
    if(e.hitWall) e.dir*=-1;
  }
  /* ---- 末影人: 周期瞬移 + 仅近身追击(被箭击中会瞬移闪避) ---- */
  else if(e.type==='enderman'){
    e.tele--;
    if(e.tele<=0){ e.tele=rand(100,170); endermanTeleport(e); }
    if(Math.abs(dist)<130 && !player.dead){ e.dir=sign(dist)||e.dir; e.vx=e.dir*e.speed; }
    else e.vx*=0.85;
    e.hitWall=false; moveEntity(e);
    if(e.hitWall) e.dir*=-1;
  }
  /* ---- 烈焰人: 飞行悬浮 + 三连火球 ---- */
    ...
    e.vx += (sign(player.x-e.x)*1.0 - e.vx)*0.05;
    const desiredY = player.y-100 + Math.sin(e.t*0.05)*16;
    e.vy += (clamp((desiredY-e.y)*0.04,-2.6,2.6) - e.vy)*0.2;
    e.x+=e.vx; e.y+=e.vy; if(e.y<20) e.y=20;
    e.shoot--;
    if(Math.abs(dist)<470 && Math.abs(player.y-e.y)<260 && !player.dead && e.shoot<=0){
      e.shoot=rand(78,118);
      for(let i=-1;i<=1;i++){ const sx=e.x+e.w/2, sy=e.y+e.h/2;
        const ang=Math.atan2((player.y+player.h/2)-sy,(player.x+player.w/2)-sx)+i*0.2;
        projectiles.push({x:sx,y:sy,vx:Math.cos(ang)*4.6,vy:Math.sin(ang)*4.6,w:16,h:16,
          kind:'fireball',life:130,from:'enemy'}); }
      SFX.arrow();
    }
  }
  /* ---- 蜘蛛: 快速冲撞 + 撞墙向上攀爬 ---- */
  else if(e.type==='spider'){
    if(!player.dead) e.dir=sign(dist)||e.dir;
    e.vx=e.dir*e.speed;
    e.hitWall=false; moveEntity(e);
    if(e.hitWall){ if(player.y < e.y+e.h){ e.vy=-3.4; e.climb=12; } else e.dir*=-1; }
    else if(e.onGround && e.climb<=0){
      const fc=Math.floor((e.x+(e.dir>0?e.w+2:-2))/TILE), fr=Math.floor((e.y+e.h+2)/TILE);
      if(!solidAt(fc,fr) && Math.random()<0.25) e.vy=-7;   // 蜘蛛会跳过小坑
    }
    if(e.climb>0) e.climb--;
  }
}
function endermanTeleport(e){
  burst(e.x+e.w/2,e.y+e.h/2,'#b24bff',12,4);
  const side=Math.random()<0.5?-1:1;
  const tx=clamp(player.x+side*rand(90,190), 20, G.lvl.pxW-40);
  e.x=tx; e.y=groundY(tx+e.w/2)-e.h; e.vy=0;
  burst(e.x+e.w/2,e.y+e.h/2,'#b24bff',12,4); blip(640,0.08,'sine',0.1,-260);
}
function splitSlime(e){
  if(e.type!=='slime' || e.size<=1) return;
  for(let i=-1;i<=1;i+=2){
    spawnEnemy({type:'slime', x:e.x+e.w/2-10, y:e.y, size:e.size-1});
    const child=enemies[enemies.length-1]; child.vx=i*2.5; child.vy=-6; child.dir=i; child.hop=rand(10,30);
  }
}
function creeperExplode(e){
  e.dead=true; SFX.explode(); G.shake=14; flash(0.3,'255,150,50'); hitStop(4); vibrate(50);
...
function damagePlayer(amount, fromX){
  ...
  popText(player.x+player.w/2, player.y-4, '-'+amount, '#ff5a5a', 17);
  hitStop(3); flash(0.28,'255,40,40'); vibrate(35); G.combo=0; G.comboT=0;
  if(player.hp<=0){
    if(player.totem){   // 不死图腾: 消耗后复活, 不死
      player.totem=false; player.hp=6; player.iframe=130;
      SFX.heal(); flash(0.55,'255,230,120'); hitStop(6); vibrate(120);
      popText(player.x+player.w/2, player.y-12, '不死图腾! 复活', '#ffd24a', 20);
      burst(player.x+player.w/2, player.y+player.h/2, '#ffd24a', 30, 6);
    } else { player.hp=0; killPlayer(); }
  }
}
function killPlayer(){
...
const POTION_DUR = { speed:480, strength:480, fireres:600 };
function applyPotion(kind, x, y){
  SFX.heal(); flash(0.2,'255,255,255'); burst(x,y,'#fff',12,4); vibrate(20);
  if(kind==='speed'){    player.speedT   =Math.max(player.speedT,   POTION_DUR.speed);    popText(x,y,'⚡速度!','#5fd0ff',17); }
  else if(kind==='strength'){ player.strengthT=Math.max(player.strengthT,POTION_DUR.strength); popText(x,y,'⚔力量!','#ff7a4a',17); }
  else if(kind==='fireres'){  player.fireresT =Math.max(player.fireresT, POTION_DUR.fireres);  popText(x,y,'🔥抗火!','#ffb24a',17); }
  else if(kind==='totem'){    player.totem=true;                                               popText(x,y,'☥不死图腾','#ffd24a',17); }
}
...
/* --- v1.3 打击感: 顿帧 / 闪光 / 震动 --- */
function hitStop(frames){ if(frames>G.hitStop) G.hitStop=frames; }       // 冻结逻辑若干真实帧
function flash(amount,color){ if(amount>G.flash){ G.flash=amount; G.flashColor=color||'255,255,255'; } }
function vibrate(ms){ try{ if(navigator && navigator.vibrate) navigator.vibrate(ms); }catch(e){} }

/* --- 伤害飘字 --- */
function popText(x,y,text,color,size){
  floatTexts.push({ x,y, vy:-1.1, life:44, max:44, text:String(text), color:color||'#fff', size:size||16 });
}
function updateFloatTexts(){
  for(const f of floatTexts){ f.y+=f.vy; f.vy*=0.92; f.life--; }
  floatTexts = floatTexts.filter(f=>f.life>0);
}
function drawFloatTexts(){
  for(const f of floatTexts){
    const sx=f.x+G.cam.x, sy=f.y+G.cam.y, a=clamp(f.life/f.max,0,1);
    X.globalAlpha=a; X.textAlign='center';
    X.font='bold '+f.size+'px "Microsoft YaHei"';
    X.lineWidth=3; X.strokeStyle='rgba(0,0,0,.85)'; X.strokeText(f.text,sx,sy);
    X.fillStyle=f.color; X.fillText(f.text,sx,sy);
  }
...
/* --- 连击 --- */
function addCombo(){
  G.combo++; G.comboT=110;
  if(G.combo>=2){ const bonus=(G.combo-1)*5; G.score+=bonus; }
  return G.combo;
}
function updateCombo(){ if(G.comboT>0){ G.comboT--; if(G.comboT<=0) G.combo=0; } }
...
function bossHurt(dmg){
  ...
  popText(boss.x+boss.w/2, boss.y-2, '-'+dmg, '#ffd24a', 20);
  hitStop(3); vibrate(20);
  if(boss.hp<=0) defeatBoss();
}
function defeatBoss(){
  boss.hp=0; boss.dead=true; boss.defeated=true; SFX.win(); G.shake=20;
  hitStop(9); flash(0.5,'255,255,255'); vibrate(140);
...
function updateProjectiles(){
  for(const p of projectiles){
    ...
    // 玩家弓箭命中怪物 / Boss
      ...
      const admg = player.strengthT>0?2:1;
      for(const e of enemies){
        if(!e.dead && aabb(p,e)){
          if(e.type==='enderman'){ endermanTeleport(e); p.life=0; break; }   // 末影人瞬移闪避箭矢
          e.hp-=admg; e.flash=8; SFX.hit(); burst(e.x+e.w/2,e.y+e.h/2,'#fff',6,4);
          e.vx=sign(p.vx)*3;
          if(e.hp<=0){ e.dead=true; killReward(e); } else popText(e.x+e.w/2,e.y-2,'-'+admg,'#fff',13);
          p.life=0; break;
        }
      }
      if(p.life>0 && boss && !boss.dead && boss.state==='fight' && aabb(p,boss)){
        bossHurt(player.strengthT>0?2:1); p.life=0;
      }
    }
    // 撞墙消失(箭/骷髅头/火球)
    if((p.kind==='arrow'||p.kind==='skull'||p.kind==='fireball') &&
        solidAt(Math.floor((p.x+p.w/2)/TILE),Math.floor((p.y+p.h/2)/TILE))){
      if(p.kind==='skull'){ burst(p.x,p.y,'#444',6,3); }
      if(p.kind==='fireball'){ burst(p.x,p.y,'#ff7a30',8,4); } p.life=0;
    }
  }
...
function tntExplode(tr){
  ...
  SFX.explode(); G.shake=16; flash(0.32,'255,120,40'); hitStop(4); vibrate(60);
...
function updatePlayer(){
  ...
  if(p.strengthT>0) p.strengthT--;
  if(p.fireresT>0) p.fireresT--;

  if(p.win || p.dead){ p.vy+=0.6; moveEntity(p); return; }

  const STR = p.strengthT>0;                       // 力量: 近战/远程 +1 伤害
  const ground = standingTile(p);                  // 脚下瓦片(地面属性)
  const onIce  = ground===ICE_TILE && p.onGround;
  const onSoul = ground===SOUL_TILE && p.onGround;
  const spdMul = p.speedT>0?1.5:1;
  const accel = (onIce?0.2:0.7)*spdMul;
  const maxSpd = (p.dash>0?8.5:4.2)*(onSoul?0.5:1)*spdMul;
  const fric = onIce?0.96:0.78;                    // 冰面打滑(摩擦极低)
  let move=0;
  ...
  // 冲刺: Shift 键, 或 双击方向(移动端无 Shift, 双击左/右即冲刺)
  let dashReq = tapped('ShiftLeft','ShiftRight');
  if(tapped('ArrowRight','KeyD')){ if(p.dblR>0) dashReq=true; p.dblR=16; }
  if(tapped('ArrowLeft','KeyA')){  if(p.dblL>0) dashReq=true; p.dblL=16; }
  if(p.dblR>0) p.dblR--; if(p.dblL>0) p.dblL--;
  if(dashReq && p.dashCD<=0 && move){ p.dash=12; p.dashCD=45; SFX.djump(); vibrate(15); }
  ...
  if(p.jumpBuf>0 && (p.onGround || p.coyote>0)){      // 地面/土狼跳(缓冲)
    p.vy=-12.5; p.jumps=1; p.jumpBuf=0; p.coyote=0; SFX.jump(); vibrate(12);
  } else if(jumpTap && !p.onGround && p.coyote<=0 && p.jumps>=1 && p.jumps<2){  // 空中二段跳
    p.vy=-11; p.jumps=2; p.jumpBuf=0; SFX.djump(); burst(p.x+p.w/2,p.y+p.h,'#fff',8,3); vibrate(10);
  }
  // 长按跳更高
  ...
  // 攻击(挥剑) — 按住可连续挥(冷却控制频率, 手机上不用狂点)
  if(down('KeyJ','Attack') && p.attackCD<=0){
  ...
  if(p.onGround){ p.jumps=0; p.coyote=7;     // 着地刷新土狼时间
    // 黏液块: 下落着陆反弹(高度随下落速度)
    if(standingTile(p)===SLIME_TILE && fallV>2){
      p.vy=-Math.min(19, 8+fallV*0.95); p.onGround=false; SFX.jump();
      burst(p.x+p.w/2,p.y+p.h,'#7ad55a',8,4); vibrate(12);
    }
  }
  // Boss 战把玩家限制在竞技场内
  ...
  // 致命方块(抗火药水: 岩浆免疫, 尖刺仍致命)
  if(touchingDeadly(p)){
    if(!(p.fireresT>0 && !touchingSpikes(p))){ damagePlayer(2, p.x); if(!p.dead){ p.vy=-9; } }
  }
  ...
  if(p.attackT>6){
    ...
    const mdmg = STR?2:1;
    for(const e of enemies){ if(!e.dead && aabb(hb,e)){ e.hp-=mdmg; e.flash=8; SFX.hit();
        burst(e.x+e.w/2,e.y+e.h/2,'#fff',6,4); e.vx=p.dir*4; vibrate(8);
        if(e.hp<=0){ e.dead=true; killReward(e);} else popText(e.x+e.w/2,e.y-2,'-'+mdmg,'#fff',13); } }
    if(boss && !boss.dead && boss.state==='fight' && aabb(hb,boss)) bossHurt(STR?3:2);
  }

  // 踩踏击杀(下落且踩在怪物头上) — 踩踏成功给短暂无敌, 不会被接触伤害反伤
  for(const e of enemies){ if(e.dead) continue;
    if(p.vy>0 && aabb(p,e) && (p.y+p.h) < e.y+e.h*0.6){
      e.hp-=(STR?3:2); p.vy=-10; p.iframe=Math.max(p.iframe,14); SFX.stomp(); burst(e.x+e.w/2,e.y,'#fff',8,4);
  ...
  if(boss && !boss.dead && boss.state==='fight' && p.vy>0 && aabb(p,boss) && (p.y+p.h)<boss.y+boss.h*0.5){
    bossHurt(STR?3:2); p.vy=-12; p.iframe=Math.max(p.iframe,22); }
  ...
  for(const o of G.lvl.emeralds){ if(!o.got && aabb(p,o)){ o.got=true; G.score+=10; SFX.coin();
      burst(o.x,o.y,'#3df58a',10,4); popText(o.x+12,o.y,'+10','#3df58a',14);} }
  for(const a of G.lvl.apples){ if(!a.got && aabb(p,a)){ a.got=true; healPlayer(2);
      popText(a.x+14,a.y,'+❤','#7cff9a',16); } }
  for(const it of G.lvl.items){ if(!it.got && aabb(p,it)){ it.got=true; applyPotion(it.kind, it.x+12, it.y); } }
...
function killReward(e){
  ...
  const c=addCombo();                          // 连击 +bonus(addCombo 内已计入分数)
  const gained = 25 + (c>=2?(c-1)*5:0);
  popText(e.x+e.w/2, e.y-4, '+'+gained, c>=4?'#ffd24a':'#7cff9a', c>=4?18:15);
  hitStop(2); vibrate(12);
  if(e.type==='creeper'&&e.armed){ creeperExplode(e); }
  if(e.type==='slime'){ splitSlime(e); }
...
function initRun(){
  ...
  enemies=[]; projectiles=[]; particles=[]; boss=null; floaters=[]; floatTexts=[];
  for(const s of G.lvl.spawns) spawnEnemy(s);
  scanTraps();
  G.bossLock=false; G.bossLockX=0; G.cam={x:0,y:0};
  G.paused=false; G.overlay=null;
  G.hitStop=0; G.look=0; G.camSnap=true; G.flash=0; G.combo=0; G.comboT=0;
  G.fade=1;   // 关卡/竞技场切入: 黑场淡入
}
function loadLevel(i){
  G.mode='story'; G.levelIndex=i; G.lvl=buildLevel(LEVELS[i]); initRun();
  G.message=LEVELS[i].name; G.msgT=160;
}
/* 模式: 平台竞技场(Boss Rush / 无尽生存共用) */
function buildArena(theme){
  const W=24,H=14, map=[];
  for(let r=0;r<H;r++){ let row='';
    for(let c=0;c<W;c++){
      if(r>=12) row+=theme.floor;
      else if(c===0||c===W-1) row+='b';
      else row+=' ';
    }
    map.push(row.split(''));
  }
  return { def:{name:theme.name,boss:theme.boss,sky:theme.sky}, W,H,map,
           pxW:W*TILE, pxH:H*TILE, emeralds:[],apples:[],items:[],spawns:[],
           bossTrigger:null, start:{x:120,y:H*TILE-2*TILE-40} };
}
function startStory(){ G.score=0; loadLevel(0); G.state='play'; }
function startBossRush(){
  G.mode='bossrush'; G.state='play'; G.score=0; G.brIndex=0; G.brTime=0;
  G.lvl=buildArena(ARENA_THEMES[0]); initRun();
  G.bossLock=true; G.bossLockX=0; spawnBoss(BOSSES[0]);
  G.message='⚔ Boss Rush 开始!'; G.msgT=120;
}
function startEndless(){
  G.mode='endless'; G.state='play'; G.score=0; G.wave=0;
  G.lvl=buildArena({name:'无尽生存',boss:'creeper',sky:['#160f28','#34204f'],floor:'s'}); initRun();
  endlessWave();
}
function endlessWave(){
  G.wave++; SFX.boss();
  G.message='⚡ 第 '+G.wave+' 波'; G.msgT=90;
  const n=Math.min(3+G.wave, 12);
  const pool=['zombie','creeper','skeleton','slime','spider'];
  if(G.wave>=3) pool.push('enderman');
  if(G.wave>=5) pool.push('blaze');
  for(let i=0;i<n;i++){
    const t=pool[Math.floor(rand(0,pool.length))];
    spawnEnemy({type:t, x:rand(90,G.lvl.pxW-90), y:40});
  }
  if(G.wave%3===0) G.lvl.apples.push({x:G.lvl.pxW/2-14,y:210,w:28,h:28,t:0,got:false});
}
function nextLevel(){
  if(G.mode==='bossrush'){
    G.brIndex++;
    if(G.brIndex<BOSSES.length){
      G.lvl=buildArena(ARENA_THEMES[G.brIndex]); initRun();
      G.bossLock=true; G.bossLockX=0; spawnBoss(BOSSES[G.brIndex]);
      G.state='play'; SFX.win();
...
function drawBlock(ch,x,y){
  ...
  switch(ch){
    ...
    case '^': { // 尖刺
      ...
      break; }
    case 'J': { // 黏液块(弹跳)
      X.fillStyle='#5fae4f'; X.fillRect(x,y,t,t);
      X.fillStyle='rgba(150,240,140,.5)'; X.fillRect(x+4,y+4,t-8,t-8);
      X.strokeStyle='#3f7a35'; X.lineWidth=1; X.strokeRect(x+.5,y+.5,t-1,t-1);
      X.fillStyle='rgba(255,255,255,.4)'; X.fillRect(x+6,y+6,9,9); break; }
    case 'G': { // 灵魂沙(减速)
      X.fillStyle='#5a4636'; X.fillRect(x,y,t,t); noiseTex(x,y,t,'#3e2f24');
      X.fillStyle='#241b14';
      X.beginPath(); X.arc(x+13,y+16,4,0,7); X.arc(x+27,y+16,4,0,7); X.fill();
      X.fillRect(x+12,y+26,16,4); break; }
    case 'I': { // 蓝冰(打滑)
      X.fillStyle='#9fd8ff'; X.fillRect(x,y,t,t);
      X.fillStyle='#c8ecff'; X.fillRect(x,y,t,6);
      X.strokeStyle='#7fc0ee'; X.lineWidth=1; X.strokeRect(x+.5,y+.5,t-1,t-1);
      X.strokeStyle='rgba(255,255,255,.6)'; X.lineWidth=2;
      X.beginPath(); X.moveTo(x+7,y+31); X.lineTo(x+21,y+8); X.stroke(); break; }
  }
}
...
function drawEnemy(e){
  ...
  if(e.type==='zombie'){
    ...
    const s=e.w;
    X.fillStyle='rgba(95,174,79,.9)'; X.fillRect(-s/2,0,s,s);
    X.fillStyle='rgba(160,240,140,.5)'; X.fillRect(-s/2+3,3,s-6,s-6);
    X.fillStyle='#163a16';
    X.fillRect(-s*0.22,s*0.32,4,4); X.fillRect(s*0.1,s*0.32,4,4); X.fillRect(-3,s*0.56,6,3);
  }
  else if(e.type==='enderman'){
    X.fillStyle='#0b0b10'; X.fillRect(-5,18,4,26); X.fillRect(1,18,4,26);  // 长腿
    X.fillStyle='#15151f'; X.fillRect(-7,2,14,24);                          // 躯干
    X.fillStyle='#0b0b10'; X.fillRect(-9,-12,18,16);                        // 头
    X.fillStyle='#cf6bff'; X.fillRect(-6,-6,5,3); X.fillRect(2,-6,5,3);     // 紫眼
    X.fillStyle='rgba(180,90,255,.45)'; X.fillRect(-2+Math.sin(e.t*0.3)*4,-2,3,3);
  }
  else if(e.type==='blaze'){
    X.save(); X.translate(0,e.h/2); const a=e.t*0.2;
    for(let i=0;i<4;i++){ X.save(); X.rotate(a+i*Math.PI/2);
      X.fillStyle= i%2?'#ffcf3a':'#ff9a2a'; X.fillRect(-3,-19,6,12); X.restore(); }
    X.fillStyle='#3a2a10'; X.beginPath(); X.arc(0,0,11,0,7); X.fill();
    X.fillStyle='#ffec8a'; X.beginPath(); X.arc(0,0,7,0,7); X.fill();
    X.fillStyle='#000'; X.fillRect(-5,-3,3,4); X.fillRect(2,-3,3,4); X.restore();
  }
  else if(e.type==='spider'){
    X.strokeStyle='#1a0e06'; X.lineWidth=2;
    for(let i=0;i<3;i++){ const lx=-8+i*8, sw=Math.sin(e.anim+i)*3;
      X.beginPath(); X.moveTo(lx,9); X.lineTo(lx-7,19+sw); X.stroke();
      X.beginPath(); X.moveTo(lx,9); X.lineTo(lx-7,-1-sw); X.stroke(); }
    X.fillStyle='#3a2418'; X.fillRect(-14,4,26,14);   // 腹
    X.fillStyle='#241008'; X.fillRect(8,3,12,12);     // 头胸
    X.fillStyle='#ff3b3b'; X.fillRect(11,6,3,3); X.fillRect(16,6,3,3);
  }
  X.restore();
}
...
function drawProjectiles(){
  for(const p of projectiles){
    ...
    if(p.kind==='arrow'){ X.save(); X.translate(sx,sy); X.rotate(Math.atan2(p.vy,p.vx));
      ...
      X.fillStyle='rgba(255,120,40,.4)'; X.beginPath(); X.arc(sx-(p.vx||0),sy-(p.vy||0),7,0,7); X.fill();
      X.fillStyle='#ff7a20'; X.beginPath(); X.arc(sx,sy,9,0,7); X.fill();
      X.fillStyle='#ffe27a'; X.beginPath(); X.arc(sx,sy,5,0,7); X.fill(); }
  }
}
...
function drawHUD(){
  ...
  let info;
  if(G.mode==='bossrush') info='Boss Rush '+Math.min(G.brIndex+1,4)+'/4 · '+((G.brTime/60)|0)+'s';
  else if(G.mode==='endless') info='无尽生存 · 第 '+G.wave+' 波';
  else info='关卡 '+(G.levelIndex+1)+' / '+LEVELS.length;
  X.fillText(info, VW-176, 63);
  // 静音指示
  ...
  if(player){
    const buffs=[];
    if(player.speedT>0)    buffs.push(['速','#5fd0ff',player.speedT]);
    if(player.strengthT>0) buffs.push(['力','#ff7a4a',player.strengthT]);
    if(player.fireresT>0)  buffs.push(['抗','#ffb24a',player.fireresT]);
    if(player.totem)       buffs.push(['腾','#ffd24a',-1]);
    let bx=14; const by=SETTINGS.muted?72:44;
    for(const [ic,col,tt] of buffs){
      X.fillStyle='rgba(0,0,0,.45)'; X.fillRect(bx,by,46,22);
      X.fillStyle=col; X.font='bold 14px "Microsoft YaHei"'; X.textAlign='left'; X.fillText(ic, bx+5, by+16);
      if(tt>0){ X.fillStyle='#fff'; X.font='10px monospace'; X.fillText(Math.ceil(tt/60)+'s', bx+22, by+15); }
      else { X.fillStyle='#fff'; X.font='10px monospace'; X.fillText('∞', bx+24, by+15); }
      bx+=52;
    }
  }
}
...
function drawCollectibles(){
  ...
  for(const it of G.lvl.items){ if(it.got) continue; it.t++;
    const x=it.x+G.cam.x, y=it.y+G.cam.y+Math.sin(it.t*0.09)*3;
    if(it.kind==='totem'){
      X.fillStyle='#e6c34a'; X.fillRect(x+6,y+4,12,22);
      X.fillStyle='#1a6a3a'; X.fillRect(x+8,y+11,8,4);
      X.fillStyle='#caa23a'; X.fillRect(x+2,y+9,4,8); X.fillRect(x+18,y+9,4,8);
      X.fillStyle='#fff2b0'; X.fillRect(x+9,y+6,6,4);
    } else {
      const col = it.kind==='speed'?'#5fd0ff': it.kind==='strength'?'#ff6a3a':'#ffb24a';
      X.fillStyle='#cdd6e0'; X.fillRect(x+9,y+1,6,5);
      X.fillStyle=col; X.beginPath(); X.arc(x+12,y+16,9,0,7); X.fill();
      X.fillStyle='rgba(255,255,255,.5)'; X.fillRect(x+6,y+11,3,7);
      X.strokeStyle='#9fb0c0'; X.lineWidth=1; X.beginPath(); X.arc(x+12,y+16,9,0,7); X.stroke();
    }
    X.strokeStyle='rgba(255,255,255,.22)'; X.lineWidth=1; X.beginPath(); X.arc(x+12,y+14,14,0,7); X.stroke();
  }
}
...
function drawBackground(){
  let sky=G.lvl.def.sky;
  if(G.lvl.def.boss==='creeper'){      // 草原: 昼夜循环
    const night=Math.sin(G.t*0.0016)*0.5+0.5; G._night=night;
    sky=[ lerpColor(sky[0],'#0a1430',night*0.82), lerpColor(sky[1],'#16244a',night*0.82) ];
  } else G._night=0;
  ...
  if(G.lvl.def.boss==='dragon'){ // 末地黑曜石柱 + 星
    ...
    for(let i=0;i<26;i++){ const sx=((i*123 - par*0.4)%VW+VW)%VW, sy=(i*71+G.t*0.3)%VH;
      X.fillRect(sx,sy,2,2); }
  } else { // 草原:云(+夜晚星星, 黄昏太阳/月亮)
    if(G._night>0.32){ X.fillStyle='rgba(255,255,255,'+clamp((G._night-0.32)*1.1,0,.8)+')';
      for(let i=0;i<40;i++){ const sx=((i*127 - par*0.12)%VW+VW)%VW, sy=(i*61)%(VH*0.55); X.fillRect(sx,sy,2,2); } }
    // 太阳(白天)/月亮(夜晚)
    const sunX=VW*0.78, sunY=90+G._night*40;
    if(G._night<0.55){ X.fillStyle='rgba(255,235,150,'+(0.95-G._night)+')'; X.beginPath(); X.arc(sunX,sunY,26,0,7); X.fill(); }
    else { X.fillStyle='rgba(232,237,246,.92)'; X.beginPath(); X.arc(sunX,sunY,22,0,7); X.fill();
      X.fillStyle='rgba(200,210,225,.5)'; X.beginPath(); X.arc(sunX-6,sunY-4,4,0,7); X.arc(sunX+7,sunY+5,3,0,7); X.fill(); }
    X.fillStyle='rgba(255,255,255,'+(0.85-G._night*0.45)+')';
...
function drawAtmosphere(){
  const boss=G.lvl.def.boss, t=G.t;
  if(boss==='creeper'){            // 草原: 飘落花粉
    X.fillStyle='rgba(255,255,210,.5)';
    for(let i=0;i<26;i++){ const x=((i*137 + t*0.4)%(VW+40)+VW+40)%(VW+40)-20;
      const y=((i*89 + t*1.0)%(VH+40)); X.fillRect(x,y,2,2); }
  } else if(boss==='wither'){      // 下界: 上升火星
    for(let i=0;i<24;i++){ const x=((i*101 + Math.sin((t+i*40)*0.02)*22)%(VW+40)+VW+40)%(VW+40)-20;
      const y=VH-((i*67 + t*1.5)%(VH+40))+20;
      X.fillStyle='rgba(255,'+(120+(i%3)*45)+',40,.6)'; X.fillRect(x,y,2,3); }
  } else if(boss==='dragon'){      // 末地: 紫色微尘
    X.fillStyle='rgba(200,120,255,.5)';
    for(let i=0;i<24;i++){ const x=((i*149 - t*0.3)%(VW+40)+VW+40)%(VW+40)-20;
      const y=((i*97 + Math.sin((t+i*30)*0.03)*30 + t*0.5)%(VH+40)+VH+40)%(VH+40)-20;
      X.fillRect(x,y,2,2); }
  } else if(boss==='warden'){      // 深暗: 漂浮幽蓝孢子(加密)
    X.fillStyle='rgba(60,230,200,.45)';
    for(let i=0;i<22;i++){ const x=((i*113 + Math.sin((t+i*50)*0.02)*16)%(VW+40)+VW+40)%(VW+40)-20;
      const y=((i*83 - t*0.5)%(VH+40)+VH+40)%(VH+40)-20; X.fillRect(x,y,2,2); }
  }
}
/* 氛围暗角(越阴暗的世界越重; 深暗有光照遮罩故此处减弱) */
function drawVignette(){
  const boss=G.lvl.def.boss;
  const s = boss==='warden'?0.28 : boss==='dragon'?0.42 : boss==='wither'?0.34 : 0.18;
  const g=X.createRadialGradient(VW/2,VH/2, VH*0.34, VW/2,VH/2, VH*0.92);
  g.addColorStop(0,'rgba(0,0,0,0)'); g.addColorStop(1,'rgba(0,0,0,'+s+')');
  X.fillStyle=g; X.fillRect(0,0,VW,VH);
}
/* 深暗之域: 跟随玩家的光照遮罩 — 只照亮周围一圈, 越远越黑 */
function drawLightMask(shx,shy){
  if(G.lvl.def.boss!=='warden') return;
  const px=player.x+player.w/2+G.cam.x+shx, py=player.y+player.h/2+G.cam.y+shy;
  const r0=72, r1=240;
  const g=X.createRadialGradient(px,py,r0, px,py,r1);
  g.addColorStop(0,'rgba(2,6,10,0)'); g.addColorStop(1,'rgba(2,6,10,0.94)');
  X.fillStyle=g; X.fillRect(0,0,VW,VH);
  const gl=X.createRadialGradient(px,py,0, px,py,r0);
  gl.addColorStop(0,'rgba(120,220,200,0.12)'); gl.addColorStop(1,'rgba(120,220,200,0)');
  X.fillStyle=gl; X.fillRect(px-r0,py-r0,r0*2,r0*2);
}
/* 颜色插值(昼夜循环用) */
function hex2rgb(h){ h=h.replace('#',''); if(h.length===3) h=h.split('').map(c=>c+c).join('');
  return [parseInt(h.slice(0,2),16),parseInt(h.slice(2,4),16),parseInt(h.slice(4,6),16)]; }
function lerpColor(a,b,t){ const pa=hex2rgb(a), pb=hex2rgb(b);
  return 'rgb('+Math.round(pa[0]+(pb[0]-pa[0])*t)+','+Math.round(pa[1]+(pb[1]-pa[1])*t)+','+Math.round(pa[2]+(pb[2]-pa[2])*t)+')'; }
/* 连击数显示(屏幕中上, 越高越大越黄) */
function drawCombo(){
  if(G.combo<2) return;
  const a=clamp(G.comboT/110,0,1);
  const sz=22+Math.min(G.combo,12)*1.6, pop=1+Math.max(0,(G.comboT-95)/15)*0.25;
  X.save(); X.translate(VW/2, 96); X.scale(pop,pop);
  X.globalAlpha=clamp(a+0.3,0,1); X.textAlign='center';
  X.font='bold '+sz+'px "Microsoft YaHei"';
  X.lineWidth=4; X.strokeStyle='rgba(0,0,0,.8)';
  const txt='连击 ×'+G.combo;
  X.strokeText(txt,0,0); X.fillStyle= G.combo>=6?'#ff5a2a': G.combo>=4?'#ffd24a':'#fff';
  X.fillText(txt,0,0); X.restore(); X.globalAlpha=1; X.textAlign='left';
}
...
function render(){
  ...
  if(G.camSnap){ G.cam.x=targetX; G.cam.y=targetY; G.camSnap=false; }
  else { G.cam.x += (targetX-G.cam.x)*0.16; G.cam.y += (targetY-G.cam.y)*0.22; }
  ...
  drawFloaters();
  drawFloatTexts();           // 伤害/得分飘字(世界坐标)
  drawLightMask(shx,shy);     // 深暗之域: 跟随玩家的光照遮罩(只照亮周围)
  drawVignette();             // 氛围暗角
  if(G.flash>0){ X.fillStyle='rgba('+G.flashColor+','+clamp(G.flash,0,0.6)+')'; X.fillRect(0,0,VW,VH); }
  if(boss) drawBossBar();     // 屏幕坐标(HUD层), 摄像机变换已恢复
  drawHUD();
  drawCombo();
  if(G.fade>0){ X.fillStyle='rgba(0,0,0,'+clamp(G.fade,0,1)+')'; X.fillRect(0,0,VW,VH); }  // 过场淡入
...
function drawTitle(){
  ...
  X.fillStyle='#eaf6ff'; X.font='15px "Microsoft YaHei"';
  X.fillText('击败 4 大 Boss：苦力怕之王 · 凋灵 · 监守者 · 末影龙', VW/2, 286);
  if(SAVE.best>0){ X.fillStyle='#ffe680'; X.font='bold 15px "Microsoft YaHei"';
    X.fillText('🏆 最高分 '+SAVE.best, VW/2, 312); }
  // 模式选择按钮
  for(const b of titleButtons()){ const sel=b.i===G.titleSel;
    X.fillStyle= sel?'rgba(95,208,255,.28)':'rgba(0,0,0,.34)';
    roundRect(b.x,b.y,b.w,b.h,10); X.fill();
    if(sel){ X.strokeStyle='#5fd0ff'; X.lineWidth=2; roundRect(b.x,b.y,b.w,b.h,10); X.stroke(); }
    X.fillStyle= sel?'#fff':'#cfe'; X.font='bold 18px "Microsoft YaHei"'; X.textAlign='center';
    X.fillText(b.label, VW/2, b.y+28);
  }
  X.fillStyle='#9fd0ff'; X.font='12px "Microsoft YaHei"'; X.textAlign='center';
  X.fillText('↑↓ 选模式 · Enter/点击 开始 · ←→ 移动 空格 跳 J 砍(按住连) K 射 Shift/双击 冲刺 · P 暂停', VW/2, 520);
  X.textAlign='left';
}
function titleButtons(){
  const labels=['冒险模式 · 4 章 4 Boss','Boss Rush · 连战 4 Boss 计时','无尽生存 · 波次挑战'];
  const bw=420,bh=44, x=(VW-bw)/2, y0=334;
  return labels.map((label,i)=>({ i, label, x, y:y0+i*54, w:bw, h:bh }));
}
function drawEnd(win){
  resetT();
  X.fillStyle= win? 'rgba(10,40,20,.9)':'rgba(40,10,10,.9)'; X.fillRect(0,0,VW,VH);
  X.textAlign='center';
  X.fillStyle= win? '#7CFC9A':'#ff6b6b'; X.font='bold 52px "Microsoft YaHei"';
  const title = win? (G.mode==='bossrush'?'🏆 Boss Rush 通关！':'🏆 通关！你拯救了主世界！')
                   : (G.mode==='endless'?'💀 你倒下了…':'💀 你被击败了…');
  X.fillText(title, VW/2, VH/2-50);
  // 模式成绩
  X.fillStyle='#ffd24a'; X.font='bold 18px "Microsoft YaHei"';
  if(G.mode==='bossrush') X.fillText('⏱ 用时 '+((G.brTime/60)|0)+' 秒', VW/2, VH/2-18);
  else if(G.mode==='endless') X.fillText('⚡ 坚持到第 '+G.wave+' 波', VW/2, VH/2-18);
  X.fillStyle='#fff'; X.font='22px "Microsoft YaHei"';
  X.fillText('绿宝石得分：'+G.score, VW/2, VH/2+16);
  const isRecord = G.score>0 && G.score>=SAVE.best;
  X.fillStyle = isRecord? '#ffe680':'#cfe'; X.font='16px "Microsoft YaHei"';
  X.fillText(isRecord? ('🎉 新纪录！  历史最高：'+SAVE.best) : ('历史最高：'+SAVE.best), VW/2, VH/2+46);
  X.fillStyle='#cfe'; X.font='16px "Microsoft YaHei"';
  const again = G.mode==='story'? '重新开始本关' : '再来一局';
  X.fillText(win? '感谢游玩 — 按 Enter / 点击屏幕 回到标题':('按 Enter / 点击屏幕 '+again), VW/2, VH/2+80);
  X.textAlign='left';
}
...
function stepPlay(){          // 一次固定步长的游戏逻辑
  ...
  if(boss) updateBoss();
  if(G.mode==='endless' && !boss && enemies.length===0 && !player.dead && !player.win) endlessWave();
  if(G.mode==='bossrush' && boss && !boss.defeated) G.brTime++;
  updateProjectiles(); updateTraps(); updateParticles(); updateFloatTexts(); updateCombo();
  if(G.shake>0){ G.shake*=0.86; if(G.shake<0.5) G.shake=0; }
  if(G.flash>0){ G.flash-=0.06; if(G.flash<0) G.flash=0; }
  if(G.fade>0){ G.fade-=0.04; if(G.fade<0) G.fade=0; }
  if(G.msgT>0) G.msgT--;
}
function startMode(m){
  if(m===1) startBossRush();
  else if(m===2) startEndless();
  else startStory();
}
function uiTick(){            // 非游戏/暂停状态: 每帧一次 UI 逻辑
  G.t++;
  if(G.state==='title'){
    if(tapped('ArrowUp','KeyW'))   { G.titleSel=(G.titleSel+2)%3; SFX.sword(); }
    if(tapped('ArrowDown','KeyS'))  { G.titleSel=(G.titleSel+1)%3; SFX.sword(); }
    const btns=titleButtons();
    for(const b of btns) if(hit(b)) G.titleSel=b.i;
    let go=tapped('Enter','NumpadEnter');
    if(pointer.justDown){ const b=btns.find(r=>hit(r)); if(b){ G.titleSel=b.i; go=true; } }
    if(go){ audio(); startMode(G.titleSel); }
  } else if(G.state==='play' && G.paused){
    updateMenu();
  } else if(G.state==='gameover'){
    if(tapped('Enter','NumpadEnter')){
      if(G.mode==='endless') startEndless();
      else if(G.mode==='bossrush') startBossRush();
      else { G.state='play'; loadLevel(G.levelIndex); }
    }
