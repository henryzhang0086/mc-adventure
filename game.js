...
function syncMusic(){
  ...
  if(G.state==='play' && !G.paused && player && !player.dead){
    const wkey = ['overworld','nether','deepdark','end'][G.levelIndex] || 'overworld';
    const want = (boss && !boss.defeated && boss.state==='fight') ? 'boss' : wkey;
    playMusic(want);
    ...
    // 玩家弓箭命中怪物 / Boss
      ...
      for(const e of enemies){
        if(!e.dead && aabb(p,e)){
          ...
          e.vx=sign(p.vx)*3;
          if(e.hp<=0){ e.dead=true; killReward(e); } else popText(e.x+e.w/2,e.y-2,'-1','#fff',13);
          p.life=0; break;
        }
      }
...
function drawVignette(){
  ...
  const s = boss==='warden'?0.55 : boss==='dragon'?0.42 : boss==='wither'?0.34 : 0.18;
...
function drawTitle(){
  ...
  X.fillText('← → 移动 · 空格/W 跳跃(二段) · J 挥剑(按住连击) · K 射箭 · Shift/双击方向 冲刺', VW/2, 376);
